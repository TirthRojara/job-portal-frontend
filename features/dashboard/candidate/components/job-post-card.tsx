...
type JobCardProps = {
    job: JobResponseCandidate;
};

export function JobPostCard({ job }: JobCardProps) {
    const handleCardClick = () => {
        window.open(`/dashboard/candidate/job/${job.id}`, "_blank", "noopener,noreferrer");
    };
    ...
    return (
        ...
        <Card
            ...
            className="cursor-pointer w-full max-w-3xl mx-auto rounded-xl shadow-sm gap-2 py-4 hover:border-blue-300"
        >
            {/* top section */}
            <CardHeader className="flex flex-row items-start justify-between gap-3 pb-2 px-4">
                <div className="flex-1 min-w-0">
                    <CardTitle className="text-base text-md sm:text-lg font-semibold">{job.title}</CardTitle>
            ...
            <CardContent className="flex flex-col gap-3 pt-0 px-4">
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
                    ...
                    <span className="inline-flex items-center gap-1.5">
                        <CircleDollarSign className="h-4 w-4" />₹{formatSalary(job.salaryMin)} - ₹{formatSalary(job.salaryMax)}
                    </span>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className=" flex gap-1">
                        <Badge variant="secondary" className=" px-3 py-1 bg-accent  text-xs sm:text-sm">
                            {job.workplace}
                        </Badge>
                        {/* <Badge variant="secondary" className=" px-3 py-1 bg-accent  text-xs sm:text-sm">
                            {job.status}
